...
def preprocess_for_regression(df, target_column='Total Spent'):
    ...
    df_processed['Expected_Total'] = df_processed['Price Per Unit'] * df_processed['Quantity']
    
    # Ratio precio/cantidad
    df_processed['Price_Quantity_Ratio'] = df_processed['Price Per Unit'] / (df_processed['Quantity'] + 1)
    ...
    df_processed['Transaction_Size_encoded'] = le_size.fit_transform(df_processed['Transaction_Size'])
...
def prepare_features_target(df, target_column='Total Spent', exclude_columns=None):
    """
    Preparar características (X) y variable objetivo (y) para el modelo
    """
    if exclude_columns is None:
        exclude_columns = ['Transaction ID', 'Customer ID', 'Transaction Date', target_column]
    ...
    feature_columns = [col for col in df.columns if col not in columns_to_exclude and col != target_column]
